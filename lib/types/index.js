...
function indexQueries (nodeMap, indexes, oldSchema, newSchema) {
  ...
  for (let [name, { node }] of queryMap) {
    // First resolve all placeholders and type/field strings on the raw query.
  ...
  function getObjectVariables (parsed) {
    ...
    for (let triple of where) {
      if (triple.subject.startsWith('?')) vars.push(triple.subject)
      if (triple.object.startsWith('?')) vars.push(triple.object)
    }
    return vars
  }
  ...
  // the parsed object.
  // This function:
  // 1) Searches for all placeholders of the form '$name' in the raw query (these are dynamically bound later)
  // 2) Modifies all human-readable type/field strings into resolved strings
