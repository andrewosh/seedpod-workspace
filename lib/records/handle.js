...
PackageHandle.prototype.initialize = async function () {
  ...
  for (let [name, idx] of types) {
    ...
    let handle = TypeHandle(this, name)
    handle.bind()
    handles.push(handle)
  }
...
Differ.prototype.addTrigger = function (typeName, triggerHandle) {
  let typeHandle = TypeHandle(this.pkg, typeName)
  let newTrigger = {
    triggerHandle,
    typeHandle
  }
...
function TypeHandle (pkg, name) {
  if (!(this instanceof TypeHandle)) return new TypeHandle(pkg, name)
  this.pkg = pkg
  this.name = name
  this.packageName = pkg.packageName

  let [typeIdx, astIdx] = this._getIndexes(this.pkg.packageName, this.name)
  this.index = typeIdx
  this.astIndex = astIdx
...
TypeHandle.prototype._getIndexes = function (packageName, typeName) {
  let pkgIndex = this.pkg.typeIndex.get(packageName)
  return [
    pkgIndex.types.get(typeName),
    pkgIndex.index.get(typeName)
  ]
}

TypeHandle.prototype._generateTriples = async function (obj, resolved) {
  let self = this

  var index, astIndex
  if (resolved) {
    [index, astIndex] = this._getIndexes(resolved.packageName, resolved.name)
  } else {
    index = this.index
    astIndex = this.astIndex
  }
  ...
  let nestedPromises = index.fields.map(getFieldTriples)
  let mapped = [].concat(...await Promise.all(nestedPromises))
  ...
  async function getFieldTriples (field) {
    let node = astIndex.fields[field.name]
    ...
    let [fieldIndex] = fieldInfo ? self._getIndexes(fieldInfo.packageName, fieldInfo.name) : [null]

    let triplePromises = field.repeated ? value.map(makeTriples) : [makeTriples(value)]
    return Promise.all(triplePromises)
      ...
      return new Promise(async (resolve, reject) => {
        if (fieldInfo && !fieldIndex.isEnum) {
          ...
          if (!node.isTag) {
            ...
            triples = await self._generateTriples(value, fieldInfo)
            ...
            // If this is a tag type, do not reinsert the nested record, just create a new pointer to it.
            triples = {
              insertions: value._revs.map(rev => {
                return {
                  subject: rootRevs,
                  predicate: pred,
                  object: rev
...
TypeHandle.prototype.inflateObject = async function (rootId, rootRev) {
  ...
  async function inflate (rev, name, packageName) {
    let [index] = self._getIndexes(packageName, name)
    ...
    for (let { predicate, object } of rootFields) {
      ...
      if (fieldInfo && !fieldIndex.isEnum) {
        ...
        if (field.type === 'Bytes') {
          fieldPromises.push(self._loadBytes(object).then(finish))
        } else {
...
TypeHandle.prototype._loadBytes = async function (ptr) {
  let nodes = await this.pkg.fs.get(ptr)
...
TypeHandle.prototype.put = async function (ctx) {
  ...
  let putStream = through.obj(async function (chunk, enc, cb) {
    try {
      let { insertions, deletions, revs, rootId, rootRevs, blocks } = await self._generateTriples(chunk)
