...
TypeHandle.prototype._inflateObject = async function (rootId) {
  ...
  async function inflate (rev, name, packageName) {
    ...
    for (let { predicate, object } of rootFields) {
      ...
      if (fieldInfo) {
        fieldPromises.push(inflate(object, fieldInfo.name, fieldInfo.packageName).then(f => {
          if (!inflated[field.name]) inflated[field.name] = []
          // TODO: this is inherently unordered. is this acceptable?
          inflated[field.name].push(f)
        }))
      } else {
        inflated[field.name] = object
      }
    }
