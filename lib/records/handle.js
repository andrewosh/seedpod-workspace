...
TypeHandle.prototype._generateTriples = function (obj, resolved) {
  ...
  let ids = triples.filter(t => t.subject === '@types').map(t => {
    return {
      key: t.predicate,
      value: t.object
    }
...
TypeHandle.prototype._inflateObject = async function (rootId, resolved) {
  ...
  for (let { subject, predicate, object } of results) {

  }
}

TypeHandle.prototype._getAllTriples = async function (id, resolved) {
