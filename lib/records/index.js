...
function RecordManager (graph, triggers, fs, packages) {
  if (!(this instanceof RecordManager)) return new RecordManager(graph, triggers, fs, packages)
  this.graph = graph
...
RecordManager.prototype.bind = async function (packageName, packageVersion) {
  ...
  let handle = PackageHandle(this, packageName, schema, typeIndex)
