...
module.exports.compile = function (indexes, aliases, tree, manifest) {
  ...
  let methodNodes = tree.filter(n => n.nodeType === 'method')
  ...
  // This mutates `maps`
  addSelfPackage(indexes[manifest.name], typeNodes, enumNodes, queryNodes, triggerNodes, maps)
...
function addTypeByName (index, name, alias, maps) {
  ...
  function addService (nodeIndex, name) {
    let responseType = createDbResponseType(name)
    typeMap.set(responseType.type, responseType)
    dbMap.set(name, {
      name,
...
function createDbResponseType (name) {
  let responseName = pascalCase(name + 'Response')
  let type = _createType(responseName)
  type.fields = [
    {
      tag: 1,
      type: name,
