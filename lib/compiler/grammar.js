...
var grammar = {
    ...
    {"name": "SparqlQuery", "symbols": [(lexer.has("query") ? {type: "query"} : query), (lexer.has("queryName") ? {type: "queryName"} : queryName), "QueryArgs", (lexer.has("colon") ? {type: "colon"} : colon), "QueryReturn", "LeftBrace", "QueryBody"], "postprocess": 
        ([, name, args, , type, , body]) => {
          return {
            nodeType: 'query',
            name: name.value,
