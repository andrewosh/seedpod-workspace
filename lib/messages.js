...
var Map_uint32_uint32 = exports.Map_uint32_uint32 = {
  buffer: true,
  encodingLength: null,
  encode: null,
  decode: null
...
function defineTypeMetadata () {
  ...
  function encodingLength (obj) {
    ...
    length += varint.encodingLength(len)
    length += 1 + len
    if (defined(obj.latestMinor)) {
      var tmp = Object.keys(obj.latestMinor)
      for (var i = 0; i < tmp.length; i++) {
        tmp[i] = {key: tmp[i], value: obj.latestMinor[tmp[i]]}
      }
      for (var i = 0; i < tmp.length; i++) {
  ...
  function decode (buf, offset, end) {
    ...
    while (true) {
      ...
      switch (tag) {
        ...
        obj.latestMinor[tmp.key] = tmp.value
        offset += enc[1].decode.bytes
        break
        default:
        offset = skip(prefix & 7, buf, offset)
...
function defineMap_uint32_uint32 () {
  var enc = [
    encodings.varint
  ]

  Map_uint32_uint32.encodingLength = encodingLength
  Map_uint32_uint32.encode = encode
  Map_uint32_uint32.decode = decode
