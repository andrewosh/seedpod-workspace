...
function getPackageName (pkg, pkgVersion) {
  ...
  return p.join(exports.PACKAGE_ROOT, pkg, 'versions', pkgVersion)
}
...
function getSchema (pkg, pkgVersion) {
  return p.join(getPackageMount(pkg, pkgVersion), 'schema.pb')
}

function getAST (pkg, pkgVersion) {
  return p.join(getPackageMount(pkg, pkgVersion), 'ast.json')
}
...
function getLocalRootSchema () {
  return p.join(getLocalPackageRoot(), 'schema.pb')
}
