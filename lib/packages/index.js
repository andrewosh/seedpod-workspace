...
PackageManager.prototype._install = async function (key, packageVersion, opts) {
  ...
  if (!manifest) {
    manifest = await this._loadManifest(key, packageVersion)
    ...
    console.log('MOUNTING KEY:', key, 'at:', naming.packageMount(key, version.tag))
  ...
  if (dependencies) {
    ...
    for (let depName of Object.keys(dependencies)) {
      ...
      console.log('DEP INSTALLATION, after decoding, key is:', key)
...
PackageManager.prototype.install = async function (key, tag, opts) {
  ...
  if (key && !opts.local) {
    console.log('MOUNTING KEY:', key, 'at', naming.globalMount(key))
