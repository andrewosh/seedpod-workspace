...
PackageManager.prototype._getIndexes = async function (manifest) {
  ...
  for (let [depName, { schema, ast, types, local }] of depPaths) {
    let jsonCodec = codecs('json')

    schema = await this._getNoConflict(schema, jsonCodec)
    ast = await this._getNoConflict(ast, jsonCodec)
    types = this._deserializeTypeIndex(await this._getNoConflict(types))
    let index = ast ? this._indexAST(ast) : null

    // Another hack for type index nesting
    if (!local) types = types.get(depName).types

    indexes.set(depName, { schema, ast, types, index })
  }
  ...
  for (let node of filtered) {
    ...
    if (node.nodeType === 'type') {
      ...
      for (let field of node.fields) {
        idx.fields[field.fieldName] = field
      }
    }
...
PackageManager.prototype._serializeTypeIndex = function (typeIndex) {
  if (!typeIndex) return codecs('json').encode([])

  let serialized = [...typeIndex].map(([pkgName, idx]) => {
    let typeIndex = idx.types

    // This is a hack to remove one level of nesting from the type index...
    if (typeIndex.get(pkgName)) typeIndex = typeIndex.get(pkgName).types

    let copy = new Map([...typeIndex])
    copy.set('queries', [...copy.get('queries')])
    copy.set('triggers', [...copy.get('triggers')])

    return [pkgName, Object.assign({}, idx, {
      types: [...copy],
      index: [...idx.index]
    })]
  })
  return codecs('json').encode(serialized)
}

PackageManager.prototype._deserializeTypeIndex = function (raw) {
  if (!raw) return null
  let decoded = codecs('json').decode(raw)
  let typeIndex = new Map(decoded)
  typeIndex.forEach(pkgIndex => {
    pkgIndex.types = new Map(pkgIndex.types)
    pkgIndex.index = new Map(pkgIndex.index)

    pkgIndex.types.set('queries', new Map(pkgIndex.types.get('queries')))
    pkgIndex.types.set('triggers', new Map(pkgIndex.types.get('triggers')))
  })
  return typeIndex
}
...
PackageManager.prototype.export = async function (pkg, pkgVersion) {
  ...
  return {
    ...
    typeIndex: this._deserializeTypeIndex(await this._getNoConflict(typeIndexPath)),
...
PackageManager.prototype.publish = async function (tag, opts) {
  ...
  let lastVersion = !opts.skipVersioning ? manifest.version : false
  ...
  Object.assign(indexes.get(manifest.name), {
    ast,
  ...
  Object.assign(indexes.get(manifest.name), { schema })

  // TODO: implement type versioning
  let typeIndex = indexTypes(manifest.name, aliases, indexes, lastSchema, schema) || {}

  await this.db.batch([
    { key: naming.localSchema(), value: codecs('json').encode(schema) },
    { key: naming.localManifest(), value: codecs('json').encode(manifest) },
    { key: naming.localAST(), value: codecs('json').encode(ast) },
    { key: naming.localTypeIndex(), value: this._serializeTypeIndex(typeIndex) }
  ])
