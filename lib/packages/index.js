...
PackageManager.prototype._indexAST = function (ast) {
  let index = new Map()
  let filtered = ast.filter(n => (n[0].nodeType === 'type') || (n[0].nodeType === 'sparql'))
...
PackageManager.prototype.getLatestPackageFiles = async function () {
  return {
    ...
    schema: await this._getNoConflict(naming.localSchema(), codecs('json'))}
}
...
PackageManager.prototype.publish = async function (tag, opts) {
  ...
  let selfIndex = this._indexAST(parsed)
