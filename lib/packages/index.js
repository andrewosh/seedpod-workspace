...
PackageManager.prototype._getIndexes = async function (manifest) {
  ...
  const getAllDependencies = async (paths, dependencies) => {
    ...
    for (let depName of depNames) {
      ...
      let dep = dependencies[depName]
      paths.set(depName, {
        schema: naming.schema(dep.key, dep.version),
        ast: naming.ast(dep.key, dep.version),
        types: naming.typeIndex(dep.key, dep.version)
      })
      console.log('ABOUT TO GET MANIFEST FOR DEP:', dep)
      let manifest = await this._loadManifest(dep.key, dep.version)
  ...
  console.log('DEP PATHS:', depPaths)
